...
imageInfoRed=dir('031826_Cell2_Alexa594_Baclofen_Corrected*'); 
imageInfoGreen=dir('031826_Cell2_Fluo5F_Baclofen_Corrected*'); 
                                                         
savetxtFiles='y'; %save .txt files containing CaT(t), yes (y) or no

stimOnset=275; %place of first stimulus in pixels (time dimension)
samplingRate=1; %sampling rate in kHz
sweepsWithCaT=1:55; %Enter sweeps that successfully evoked CaT

redSmoothWidth=20; %filter width for red channel

ftWidth=15; %use odd number, pixels taken to calc transient as f(t)
